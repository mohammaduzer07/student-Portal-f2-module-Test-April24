...
function addStudentsData(){
    ...
    filterStudents.forEach(student =>{
        ...
        row.innerHTML = `
        <td>${student.id}</td>
        <td><img src="${student.img_src}" alt="student img"> ${student.first_name} ${student.last_name}</td>
        <td>${student.gender}</td>
        <td>${student.class}</td>
        <td>${student.marks}</td>
        <td>${student.passing ? 'Pass' : 'Fail'}</td>
        <td>${student.email}</td>
        `;
...
function sortAZ(){
    filterStudents.sort((a, b) => (a.first_name + a.last_name).localeCompare(b.first_name + b.last_name));
    addStudentsData();
}

function sortZA(){
    filterStudents.sort((a, b) => (b.first_name + b.last_name).localeCompare(a.first_name + a.last_name));
    addStudentsData();
}
...
function sortByPassing(){
    filterStudents = students.filter(student => student.passing);
    addStudentsData();
}
...
function sortByGender(){
    ...
    filterStudents = femaleStudents;
    addStudentsData();

    let maleTable = document.createElement('table');
    maleTable.innerHTML = document.getElementById('studentTable').innerHTML;
    maleTable.getElementsByTagName('thead')[0].innerHTML = '';
    document.getElementById('studentTableBody').appendChild(maleTable);

    filterStudents = maleStudents;
    addStudentsData();
}
